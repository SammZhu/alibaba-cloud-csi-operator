...
# Default the operator to the `stable` channel — keeps the regenerated bundle
# in sync with custom_manifests/04-csi-subscription.yaml in the
# alibaba-openshift sibling repo, which references `channel: stable`.
CHANNELS ?= stable
...
# csi.alibabacloud.com/alibaba-cloud-csi-operator-bundle:$VERSION and csi.alibabacloud.com/alibaba-cloud-csi-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/samzhu/alibaba-cloud-csi-operator
...
KIND_CLUSTER ?= alibaba-cloud-csi-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name alibaba-cloud-csi-operator-builder
	$(CONTAINER_TOOL) buildx use alibaba-cloud-csi-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm alibaba-cloud-csi-operator-builder
	rm Dockerfile.cross
